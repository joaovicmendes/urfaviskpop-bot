...
export class LastFmRetriever implements IMusicRetriever {
    ...
    public async fetchTopTracks(page: number): Promise<Music[]> {
        ...
        try {
            console.info(`[LastFmRetriever][fetchTopTracks] Fetching top tracks data, page ${page}`);
            ...
            console.error(`[LastFmRetriever][fetchTopTracks] Error: when fetching top tracks, page ${page}, ${error}`);
            throw(error);
        }
    }
